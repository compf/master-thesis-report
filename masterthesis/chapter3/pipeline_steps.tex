...
The steps discussed below are:
\begin{itemize}
    \item Code obtaining
    \item File filtering
    \item Data clump detection
    \item Data clump filtering
    \item Name finding
...
After retrieving the project files, a file filtering process is important. A software project consists of many artifacts like actual source code, images, libraries, or other resources. For this master's thesis, only source code files are pertinent for data clump detection. 


However, not every source code file might be analyzed for data clumps. For instance, parts of the projects must not be refactored, or a re-factorization would be too time-consuming. As outlined in chapter \ref{sec:introduction}, the cost of detecting data clumps might need to be minimized. Therefore, filtering rules might be necessary to reduce the scope of the files that are considered in the subsequent steps.  
...
The precedence between include and exclude rules can have a major impact on defining the correct rules. In the example above, exclusion rules override inclusion rules.  This means that many files can be included at large but certain files can be excluded. If however only a small set of files should be considered while excluding a larger set of files, this prevalence should be reversed so that inclusion rules dominates over exclusion rules. In this master's thesis, the latter approach will be chosen due to its  compatibility with programs like \textit{PMD} and its ability to simplify partial analysis of software projects. 
...
This step is required for some tools like the \textit{DataClumpDoctor} as comments, the method bodies, and other language parts are irrelevant for detecting data clumps. Generating an \ac{AST} that does not contain this information can speed up the detection process. This could be achieved by extracting a subset of the \ac{AST} from the source code, which includes all the relevant information to identify data clumps.
...
\subsubsection{Data clump filtering or prioritization} 
\label{subsub:filtering_data_clumps}
